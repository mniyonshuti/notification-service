...
containerID := $(shell docker ps -a -q --filter ancestor=mniyonshuti/notification-service:latest)
...
k8s-persistence-create:
	...
	echo "kubectl run -it --rm --image=mysql:5.6 --restart=Never mysql-client -- mysql -h notification-service-db -ppassword -e 'create database db_example;'"
...
k8s-info:
	...
	kubectl get pods
	echo "You can also check DB via:\n kubectl run -it --rm --image=mysql:5.6 --restart=Never mysql-client -- mysql -h notification-service-db -ppassword "
